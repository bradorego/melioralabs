...
You can host this however you want, but for easiest setup:

* `npm install`
* `gulp`

This will serve the `/dev` directory and watch `less/*.less` and `js/*.js` for any changes. Develop away.

When you're ready, `gulp build` will minify and copy the requisite files over to `/deploy` (what is actually hosted).
